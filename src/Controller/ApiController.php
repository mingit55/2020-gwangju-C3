...
class ApiController {
        ...
        json_response([
            "festivals" => array_map(function($festival){
                $festival->images = array_map(function($image){
                    return $image->local_name;
                }, DB::fetchAll("SELECT * FROM images WHERE fid = ?", [$festival->id]));
                return $festival;
    ...
    function initFestivals(){
        ...
        $festivals = simplexml_load_file(ROOT."/resources/xml/festivalList.xml");
        foreach($festivals->items->item as $item){
